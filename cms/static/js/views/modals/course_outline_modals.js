...
define(['jquery', 'backbone', 'underscore', 'gettext', 'js/views/baseview',
    ...
    HighlightsXBlockModal = CourseOutlineXBlockModal.extend({
        ...
        getIntroductionMessage: function() {
            return StringUtils.interpolate(
                gettext('The highlights you provide here are messaged (i.e., emailed) to learners. Each {item}\'s highlights are emailed at the time that we expect the learner to start working on that {item}. At this time, we assume that each {item} will take 1 week to complete.'),
                {item: this.options.xblockType}
            );
        },
